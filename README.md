...
- 세트는 `server.py`의 `GET/POST /sets`, `DELETE /sets/<id>`로 서버(`sets_data.json`)에도 저장됨 —
  같은 배포에 여러 계정으로 로그인해도 세트를 공유해서 봄. 브라우저 localStorage는 즉시반응용
  캐시로만 쓰이고, 백그라운드에서 서버와 동기화(변경된 세트만 push/delete)됨
